...
# Create the parser
parser = argparse.ArgumentParser(description="Categorize sets of pages in library to age groups.")

# Add arguments
parser.add_argument("--library_dir", type=str, default="library-pages/", help="Directory containing book jsonl files.")
parser.add_argument("--output_dir", type=str, default="cleaned-library/", help="Path to save the age category outputs.")
...
parser.add_argument("--max_context_length", type=int, default=5000, help="Maximum context length for processing pages.")
parser.add_argument("--model_name", type=str, default="mistralai/Mistral-7B-Instruct-v0.2", help="Model to use for categorizing pages.")
...
parser.add_argument("--log_file", type=str, default="voting.log", help="File name to save the logs.")
...
data = []
for file_index, book_file in enumerate(tqdm(library_files, desc="Categorizing books")):
    ...
    tokens_so_far = 0
    joined_text = ""
    start_page = file_data[0]["page_number"] if file_data else None
    for datum in file_data:
        page_number = datum["page_number"]
        text = datum["text"]
        tokens_so_far += len(tokenizer.tokenize(text))
        joined_text += text
        if tokens_so_far > args.max_context_length:
            new_datum = {"book_name": book_file_name, "start_page": start_page, "end_page": page_number, "text": joined_text}
    ...
    file_keys = [[datum["book_name"], datum["start_page"], datum["end_page"]] for datum in data]
    ...
    if pipe.is_rank_0:
        ...
        for file_key, response in zip(file_keys,responses):
            try: 
                book_name, start_page, end_page = file_key[0], file_key[1], file_key[2]
                generated_text = response.generated_text.split("<stop>")[0]
                prompt_length = response.prompt_length
                generated_length = response.generated_length
                finish_reason = response.finish_reason
                item = dict(start_page=start_page, end_page=end_page, age_category=generated_text, prompt_length=prompt_length, generated_length=generated_length, finish_reason=finish_reason)
        ...
        for book_name, book_results in results.items():
            output_file = os.path.join(args.output_dir, f"{book_name}.jsonl")
